  ...
  class Spline
  {
    public:
      ...
      Spline ( const std::vector < X >& x, const std::vector < Y >& y )
      {
        ...
        std::vector < Y > b ( n ), d ( n ), a ( n ), c ( n + 1 ), l ( n + 1 ),
          u ( n + 1 ), z ( n + 1 );
        std::vector < X > h ( n + 1 );
        ...
        for ( size_type i = 1; i < n; i++ )
        {
          ...
          u[i] = Y ( h[i] )/l[i];
          a[i] = ( Y ( 3 )/Y ( h[i] ))*( y[i + 1] - y[i] )
            - ( Y ( 3 )/Y ( h[i - 1] ))*( y[i] - y[i - 1] );
      ...
      Y interpolate ( const X& x ) const
      {
        ...
        it = std::lower_bound ( mElements.begin ( ),
                                mElements.end ( ),
                                element_type ( x ));
      ...
      std::vector < Y > interpolate ( const std::vector < X >& xx ) const
      {
        ...
        {
          it2 =
            std::lower_bound ( it2, mElements.end ( ), element_type ( *it ));
